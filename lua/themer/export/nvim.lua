...
nv.generate_nv_config = function(cp)
	...
	table.insert(conf, [[if #parts==1 and color.link then exec("hi! link " .. group .. " " .. color.link) elseif #parts ~= 1 then exec("hi " .. table.concat(parts, " ")) end end]])
	
	-- recursive highlight function
    table.insert(conf, [[local function syntax(tbl)for hl_group, hl_value in pairs(tbl) do highlight(hl_group, hl_value) end end]])

    -- load colorscheme
    table.insert(conf, [[exec("hi clear")]])
    table.insert(conf, [[if vim.fn.exists("syntax_on") then exec("syntax reset") end]])
    table.insert(conf, [[for lang, status in pairs(theme.hig_groups.langs) do if type(status) == "table" then syntax(theme.hig_groups.langs[lang]) end end]])
    table.insert(conf, [[for plugin, status in pairs(theme.hig_groups.plugins) do if type(status) == "table" then syntax(theme.hig_groups.plugins[plugin]) end end]])
